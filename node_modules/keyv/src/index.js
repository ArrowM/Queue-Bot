...
class Keyv extends EventEmitter {
	...
	keys() {
		return this.opts.store.keys()
			.then(result => result.map(k => this._stripKeyPrefix(k))) ;
	}

	values(opts) {
		let raw = opts ? opts.raw : false;
		return this.opts.store.values()
			.then(result => result.map(v => this._parseValue(v, opts)));
	}
