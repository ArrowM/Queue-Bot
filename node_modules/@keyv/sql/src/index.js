...
class KeyvSql extends EventEmitter {
	...
	keys() {
		const select = this.entry.select(this.entry.key).where(this.entry.key.like(`${this.namespace}:%`)).toString();
		return this.query(select)
			.then(rows => rows.map((r) => r.key));
	}

	values() {
		const select = this.entry.select(this.entry.value).where(this.entry.key.like(`${this.namespace}:%`)).toString();
		return this.query(select)
			.then(rows => rows.map((r) => r.value));
	}

	entries() {
		const select = this.entry.select().where(this.entry.key.like(`${this.namespace}:%`)).toString();
		return this.query(select)
			.then(rows => rows.map((r) => [r.key, r.value]));
	}
}
