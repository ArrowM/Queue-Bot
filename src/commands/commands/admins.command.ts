...
export class AdminsCommand extends EveryoneCommand {
  ...
  data = new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) => {
      subcommand.setName("get").setDescription("Get admin users and roles");
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      subcommand
      ...
      Object.values(AdminsCommand.ADD_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    })
    .addSubcommand((subcommand) => {
      ...
      Object.values(AdminsCommand.DELETE_OPTIONS).forEach((option) =>
        option.addToCommand(subcommand)
      );
      return subcommand;
    });
  ...
  static readonly ADD_OPTIONS = {
    mentionable1: new MentionableOption({
      required: true,
      id: "mentionable_1",
      description: "UUser or role to grant admin status to",
    }),
  ...
  static async admins_add(inter: SlashInteraction) {
    ...
    await inter.respond(
      `Granted Queue Bot admin access to ${mentionablesMention(
        insertedAdmins
      )}.`,
      true
    );
  ...
  static readonly DELETE_OPTIONS = {
    admins: new AdminsOption({
      required: true,
      description: "User or role to revoke admin status from",
    }),
  };
  ...
  static async admins_delete(inter: SlashInteraction) {
    ...
    const deletedAdmins = AdminUtils.deleteAdmins(
      inter.store,
      admins.map((admin) => admin.id)
    );

    await inter.respond(
      `Revoked Queue Bot admin access from ${mentionablesMention(
        deletedAdmins
      )}.`,
      true
    );
